...
from jax import lax, jit, grad, pmap, random, jacrev
from jax.tree_util import tree_map, tree_reduce, tree_leaves

import optax

from couplednet_minimal import archs
from couplednet_minimal.utils import flatten_pytree
...
def _create_arch(config):
    ...
    elif config.arch_name == "CoupledMlp":
        arch = archs.CoupledMlp(**config)
...
def _create_optimizer(config):
    if config.optimizer == "Adam":
        schedule_type = getattr(config, "schedule", "exponential")
        if schedule_type == "cosine":
            max_steps = getattr(config, "cosine_decay_steps", 50000)
            alpha = getattr(config, "cosine_alpha", 0.01)
            ...
            lr = optax.exponential_decay(
                ...
                staircase=getattr(config, "staircase", False),
            )
        ...
        if warmup_steps > 0:
            ...
            lr = optax.join_schedules([warmup, lr], [warmup_steps])
...
class PINN:
    ...
    def compute_weights(self, params, batch, *args):
        ...
        elif self.config.weighting.scheme == "none":
            w = tree_map(lambda x: 1.0, mean_ntk_dict)
